    ...
    for( int i = 0; i < gnssRecs; i++ ) {
        ...
        switch( id ) {
            ...
            case GPS:
                ...
                // enable it...
                enabled = ciniparser_getboolean( gpsctlConf, "gps:enabled", true );
                ...
                minch = ciniparser_getint( gpsctlConf, "gps:minimum channels", 8 );
                maxch = ciniparser_getint( gpsctlConf, "gps:maximum channels", 16 );
            ...
            case SBAS:
                ...
                enabled = ciniparser_getboolean( gpsctlConf, "sbas:enabled", false );
                flags = (uint32_t) setBit_slBits( get_uint32_slBuffer( b, o + 4), 0, enabled );
                put_uint32_slBuffer( b, o + 4, flags );

                // set our min and max channels...
                minch = ciniparser_getint( gpsctlConf, "sbas:minimum channels", 1 );
                maxch = ciniparser_getint( gpsctlConf, "sbas:maximum channels", 3 );
                put_uint8_slBuffer( b, o + 1,  minch );  // min of 8...
                put_uint8_slBuffer( b, o + 2,  maxch );  // max of 16...
            ...
            case Galileo:
                ...
                // set our min and max channels...
                minch = ciniparser_getint( gpsctlConf, "galileo:minimum channels", 4 );
                maxch = ciniparser_getint( gpsctlConf, "galileo:maximum channels", 8 );
            ...
            case BeiDou:
                ...
                // disable everything else...
                enabled = ciniparser_getboolean( gpsctlConf, "beidou:enabled", false );
                flags = (uint32_t) setBit_slBits( get_uint32_slBuffer( b, o + 4), 0, enabled );
                put_uint32_slBuffer( b, o + 4, flags );

                // set our min and max channels...
                minch = ciniparser_getint( gpsctlConf, "beidou:minimum channels", 8 );
                maxch = ciniparser_getint( gpsctlConf, "beidou:maximum channels", 16 );
                put_uint8_slBuffer( b, o + 1,  minch );  // min of 8...
                put_uint8_slBuffer( b, o + 2,  maxch );  // max of 16...
            ...
            case IMES:
                ...
                // disable everything else...
                enabled = ciniparser_getboolean( gpsctlConf, "imes:enabled", false );
                flags = (uint32_t) setBit_slBits( get_uint32_slBuffer( b, o + 4), 0, enabled );
                put_uint32_slBuffer( b, o + 4, flags );

                // set our min and max channels...
                minch = ciniparser_getint( gpsctlConf, "imes:minimum channels", 0 );
                maxch = ciniparser_getint( gpsctlConf, "imes:maximum channels", 8 );
            ...
            case QZSS:
                ...
                // disable everything else...
                enabled = ciniparser_getboolean( gpsctlConf, "qzss:enabled", false );
                flags = (uint32_t) setBit_slBits( get_uint32_slBuffer( b, o + 4), 0, enabled );
                put_uint32_slBuffer( b, o + 4, flags );

                // set our min and max channels...
                minch = ciniparser_getint( gpsctlConf, "qzss:minimum channels", 0 );
                maxch = ciniparser_getint( gpsctlConf, "qzss:maximum channels", 3 );
                put_uint8_slBuffer( b, o + 1,  minch );  // min of 0...
            ...
            case GLONASS:
                ...
                // set our min and max channels...
                minch = ciniparser_getint( gpsctlConf, "glonass:minimum channels", 8 );
                maxch = ciniparser_getint( gpsctlConf, "glonass:maximum channels", 14 );
                put_uint8_slBuffer( b, o + 1,  minch );  // min of 8...
...
extern slReturn ubxConfigTimePulse( int fdPort, int verbosity ) {
    ...
    *buffer_slBuffer( body ) = 0;  // we only look at time pulse zero...
    ...
    put_uint16_slBuffer( b,  4, ciniparser_getint(gpsctlConf, "time pulse:antenna cable delay", 56 ));  // set the antenna cable delay to 56 ns...
    put_uint16_slBuffer( b,  6, ciniparser_getint(gpsctlConf, "time pulse:rf group delay", 20 ));  // set the RF group delay to 20 ns...
    put_uint32_slBuffer( b,  8, ciniparser_getint(gpsctlConf, "time pulse:unlocked pulse period", 1000000 ));  // set the unlocked period to 1 second (1,000,000 microseconds)...
    put_uint32_slBuffer( b, 16, ciniparser_getint(gpsctlConf, "time pulse:unlocked pulse length", 0 ));  // set the unlocked pulse length to 0 seconds...
    put_uint32_slBuffer( b, 12, ciniparser_getint(gpsctlConf, "time pulse:locked pulse period", 1000000 ));  // set the locked period to 1 second (1,000,000 microseconds)...
    put_uint32_slBuffer( b, 20, ciniparser_getint(gpsctlConf, "time pulse:locked pulse length", 500000 ));  // set the locked pulse length to 0.5 seconds (500,000 microseconds)...
    put_uint32_slBuffer( b, 24, ciniparser_getint(gpsctlConf, "time pulse:user configurable delay", 0 ));  // set the user-configurable delay to zero...
...
extern slReturn ubxConfigNavEngine( int fdPort, int verbosity ) {
    ...
    put_uint8_slBuffer(  b,  2, ciniparser_getint(gpsctlConf, "navigation engine:dynamic model", Stationary ));                  // use stationary mode...
    put_uint8_slBuffer(  b,  3, ciniparser_getint(gpsctlConf, "navigation engine:fix mode", Auto2D3D ));                         // use any mode...
    put_uint32_slBuffer( b,  4, (int) (ciniparser_getdouble(gpsctlConf, "navigation engine:fixed altitude (2d)", 0 ) * 100.0));  // fixed altitude for 2D (not used)...
    put_uint32_slBuffer( b,  8, (int) (ciniparser_getdouble(gpsctlConf, "navigation engine:fixed altitude variance (2d)", 1 ) * 10000.0));  // fixed altitude variance for 2D (1 m^2)...
    put_uint8_slBuffer(  b, 12, ciniparser_getint(gpsctlConf, "navigation engine:minimum elevation", 5 ));                       // minimum elevation is 5 degrees...
    put_uint16_slBuffer( b, 14, (int) (ciniparser_getdouble(gpsctlConf, "navigation engine:position dop mask", 10 ) * 10.0));    // position DoP mask is 10.0...
    put_uint16_slBuffer( b, 16, (int) (ciniparser_getdouble(gpsctlConf, "navigation engine:time dop mask", 10 ) * 10.0));        // time DoP mask is 10.0...
    put_uint16_slBuffer( b, 18, ciniparser_getint(gpsctlConf, "navigation engine:position accuracy mask", 100 ));                // position accuracy mask is 100 meters...
    put_uint16_slBuffer( b, 20, ciniparser_getint(gpsctlConf, "navigation engine:time accuracy mask", 300 ));                    // time accuracy mask is 300 m...
    put_uint8_slBuffer(  b, 22, ciniparser_getint(gpsctlConf, "navigation engine:static hold threshold", 0 ));                   // static hold threshold is 0 cm/s...
    put_uint8_slBuffer(  b, 23, ciniparser_getint(gpsctlConf, "navigation engine:dynamic gnss timeout", 60 ));                   // dynamic GNSS timeout is 60 seconds (not used)...
    put_uint8_slBuffer(  b, 24, ciniparser_getint(gpsctlConf, "navigation engine:threshold above c/no", 0 ));                    // number of satellites that must be above CNo threshold...
    put_uint8_slBuffer(  b, 25, ciniparser_getint(gpsctlConf, "navigation engine:c/no threshold", 0 ));                          // CNo threshold...
    put_uint16_slBuffer( b, 28, ciniparser_getint(gpsctlConf, "navigation engine:static hold max distance", 0 ));                // static hold distance threshold...
    put_uint8_slBuffer(  b, 30, ciniparser_getint(gpsctlConf, "navigation engine:utc standard", USNO_UTC ));                     // use USNO UTC...
