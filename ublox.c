...
extern slReturn ubxConfigNavEngine( int fdPort, int verbosity ) {
    ...
    put_uint32_slBuffer( b,  8,      10000 );  // fixed altitude variance for 2D (1 m^2)...
    put_uint8_slBuffer(  b, 12,          5 );  // minimum elevation is 5 degrees...
    put_uint16_slBuffer( b, 14,        100 );  // position DoP mask is 10.0...
    put_uint16_slBuffer( b, 16,        100 );  // time DoP mask is 10.0...
    put_uint16_slBuffer( b, 18,        100 );  // position accuracy mask is 100 meters...
    put_uint16_slBuffer( b, 20,        300 );  // time accuracy mask is 300 m...
    ...
    put_uint8_slBuffer(  b, 24,          0 );  // number of satellites that must be above CNo threshold...
    put_uint8_slBuffer(  b, 25,          0 );  // CNo threshold...
