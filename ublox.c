...
extern slReturn ubxConfigNavEngine( int fdPort, int verbosity ) {
    ...
    put_uint8_slBuffer(  b,  3, iniparser_getint(gpsctlConf, "navigation engine:fix mode", Auto2D3D ));                         // use any mode...
    ...
    put_uint32_slBuffer( b,  8, (int) (iniparser_getdouble(gpsctlConf, "navigation engine:fixed altitude variance (2d)", 1 ) * 10000.0));  // fixed altitude variance for 2D (1 m^2)...
    put_uint8_slBuffer(  b, 12, iniparser_getint(gpsctlConf, "navigation engine:minimum elevation", 5 ));                       // minimum elevation is 5 degrees...
    ...
    put_uint16_slBuffer( b, 18, iniparser_getint(gpsctlConf, "navigation engine:position accuracy mask", 100 ));                // position accuracy mask is 100 meters...
    put_uint16_slBuffer( b, 20, iniparser_getint(gpsctlConf, "navigation engine:time accuracy mask", 300 ));                    // time accuracy mask is 300 m...
    ...
    put_uint8_slBuffer(  b, 24, iniparser_getint(gpsctlConf, "navigation engine:threshold above c/no", 0 ));                    // number of satellites that must be above CNo threshold...
    put_uint8_slBuffer(  b, 25, iniparser_getint(gpsctlConf, "navigation engine:c/no threshold", 0 ));                          // CNo threshold...
